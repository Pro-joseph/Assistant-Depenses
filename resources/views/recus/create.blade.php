...
@section('content')
    <form id="createRecuForm" method="POST" action="{{ route('recus.store') }}" enctype="multipart/form-data">
        @csrf
...
TOTAL: 42.50€...">{{ old('texte_brut') }}</textarea>
                    ...
                    <div class="mt-lg flex items-center justify-between">
                        ...
                        <button id="submitBtn" class="px-xl py-sm bg-primary text-on-primary rounded-lg text-base font-semibold hover:bg-primary-container shadow-md active:scale-95 transition-all flex items-center gap-sm" type="submit">
                            <span id="submitText">Soumettre</span>
                            <span id="submitIcon" class="material-symbols-outlined">send</span>
                        </button>
                    </div>
                </div>
            </section>
...
@push('scripts')
    ...
    <script>
        document.getElementById('createRecuForm').addEventListener('submit', function (e) {
            e.preventDefault();
