        ...
        <section class="lg:col-span-7 flex flex-col gap-lg">
            <form method="POST" action="{{ route('recus.store') }}" enctype="multipart/form-data" class="bg-surface-container-lowest rounded-xl p-lg border border-outline-variant shadow-sm flex flex-col h-full">
                ...
                <div class="flex-1 relative group">
                    <textarea class="w-full h-full min-h-[300px] p-md bg-surface-container-low border border-outline-variant rounded-xl focus:border-primary focus:ring-2 focus:ring-primary/10 transition-all text-sm resize-none" name="texte_brut" placeholder="Ex: CARREFOUR CITY
24 RUE DE LA PAIX
TOTAL: 42.50€..."></textarea>
        ...
        <section class="lg:col-span-5 flex flex-col gap-lg">
            <div class="bg-surface-container-lowest rounded-xl p-lg border border-outline-variant shadow-sm">
                ...
                <label for="fileInput" class="border-2 border-dashed border-outline-variant rounded-xl p-xl flex flex-col items-center justify-center text-center cursor-pointer hover:border-primary hover:bg-primary-fixed/20 transition-all group relative overflow-hidden h-64">
                    <input accept="image/*,.pdf" class="hidden" id="fileInput" name="image" type="file">
                    <div>
